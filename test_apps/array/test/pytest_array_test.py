...
3. Build the test app:
   ...
   **Build for a specific target (replace `esp32s3` with your target chip: `esp32s3` or `esp32p4`):**
   ```bash
   python .gitlab/tools/build_apps.py test_apps/array/test -t esp32s3
   ```

   **Build for all CI targets (recommended for CI):**
   ```bash
   python .gitlab/tools/build_apps.py test_apps/array/test -t all
   ```
...
2. Run pytest with appropriate target and environment:
   ...
   **ESP32-S3 examples:**
   ```bash
   # Generic environment
   pytest test_apps/array/test --target esp32s3 --env generic
   ```
'''
